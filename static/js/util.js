...
define("util", function(require, exports, module) {
	...
	function parseDate(input) {
		...
		if (arr.length >= 6) {
			date.setHours(parseInt(arr[3], 10), parseInt(arr[4], 10), parseInt(arr[5],
				10), 0);
		}
		return date;
	...
	function confirm(title, msg, okEvent, cancleEvent) {
		...
		height = $("#msgbox-confirm-title").outerHeight() + $(
			...
			$("msgbox-confirm-ok").outerHeight() + parseInt($(
				"#msgbox-confirm-content").css("paddingTop")) + 50;
	...
	function popup(title, content) {
		...
		$("#msgbox-pop-content img").load(function() {
			...
			height = $("#msgbox-pop-title").outerHeight() + $("#msgbox-pop-content")
				.outerHeight();
			$("#msgbox-confirm-main").height(height);
		});
	 ...
	 * 显示进度条
	 */
	function progress(current, total, target, showPercent, showCancel,
		cancelEvent) {
		var percent = "",
			html = "",
			width = 0,
		...
		if (target.children(".ui-progress").length == 0) {
			html =
				"<div class='ui-progress'><span class='ui-progress-text'></span><span class='ui-progress-current'></div><div class='ui-progress-clear'></div><span class='ui-progress-cancel'>\u00d7</span>";
		}
		target.append(html);
		width = target.children(".ui-progress").width() * current / total;
		target.children(".ui-progress").children(".ui-progress-current").width(
			width);
		if (showPercent) {
			target.children(".ui-progress").css("height", "15px");
			target.children(".ui-progress").children(".ui-progress-current").css(
				"height", "15px");
			left = target.children(".ui-progress").position().left + target.children(
				".ui-progress").width() / 2 - 4;
			top = target.children(".ui-progress").position().top;
			target.children(".ui-progress").children(".ui-progress-text").css({
				"top": top + "px",
				"left": left + "px"
			}).text(percent);
		}
		...
		if (showCancel) {
			target.children(".ui-progress-cancel").unbind("click").bind("click",
				function() {
					if (cancelEvent) {
						cancelEvent();
					}
	...
	function filterMsg(msg, replaceExp, replaceurl) {
		...
		try {
			...
			temp.find("*").each(function() {
				//发送图片、接收到的图片
				if ($(this).hasClass("message-img") && this.tagName && this.tagName.toLowerCase() ==
					'img') {
					flag = true;
					return;
				}
				//发送的文件，被下面的
				if ($(this).parent().attr("rel") == "send-file" && this.tagName && this
					.tagName.toLowerCase() == 'img') {
					flag = true;
					return;
				}

				if ($(this).hasClass("image-file") && this.tagName && this.tagName.toLowerCase() ==
					"a") {
					//上传的图片
					filterEvents(this);
					result += $(this).wrap("<div></div>").parent().html();
					flag = true;
				} else if ($(this).attr("rel") == "send-file" && this.tagName && this.tagName
					.toLowerCase() == 'a') {
					var url = $(this).attr("href");
					var text = $(this).text();
					flag = true;
					result += "<a download='" + text + "' href='" + url +
						"' target='_blank' rel='send-file'><img src='./img/file2.png' style='vertical-align:middle;margin-right:10px;'/>" +
						text + "</a>";
				} else if ($(this).attr("rel") == "gallery" && this.tagName && this.tagName
					.toLowerCase() == 'a') {
					//客户端发送的图片
