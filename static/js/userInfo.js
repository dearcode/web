require(["util"], function(util) {
  $(function(){
    ...
    require(["util", "notification"],function(util, notification){
      ...
      $(document).click(function(){
        notification.requestPermission(function(){
          util.cookie("isOpenDesk", "1", {expires:30});
        });
      });
      ...
      if(c == "1"){
        ...
        util.cookie("isOpenSound", "1", {expires:30});
        $("#user_sound_text").text("关闭声音").removeClass("tgl");
      }
      //查询自己的状态
      ...
      get_contact_status(uid, function(data){
        if(data.body.presence!="off"){
          $("#user_state").attr("class",data.body.presence=="chat"?"i i-on":get_status_class(data.body.presence));
        }else{
          //重新登录
    ...
    $("#panel-user-signature-input").bind("blur",function(event){
      ...
      require(["util"],function(util){
        //验证不超过50个字符
        if(!!newSignature && newSignature.length>50){
          util.alert("签名修改","亲，签名长度不能超过50个字符哦！",signatureConfirm,signatureCancel);
          return ;
        }
        var uid = util.cookie("uid");
        var aid = util.cookie("aid");
        encodeSignature = encodeURI(newSignature);
        $.ajax({
          type : 'post',
          dataType : "json",
          data:{
    ...
    $("#user_login_out").bind("click",function(){
      ...
      util.confirm("确定", "您确定要退出timline网页版吗？", function(){
        update_user_presence("off","off",loginOutSuccess,loginOutFail);
      });
    });
    //声音通知
    ...
    //桌面通知
    $("#user_dest_set").bind("click",function(){
      ...
      if($("#timline-setting").length == 0){
        var html = "<div id='timline-setting'><ul class='notice-ttl'>" +
      ...
      $("#timline-setting").find("li").mouseout(function(event){
    ...
    $("#user_info").bind("click", function(){
      ...
      if(info && info.realname){
        ...
        phone = dom.find(".idcard-phone").html("手机：<input type='text' style='cursor:pointer;' title='点击修改' value='"+info.phone+"'>");
        if(info.tel){
          tel = dom.find(".idcard-tel").html("座机：<input type='text' style='cursor:pointer;' title='点击修改' readonly='readonly' value='"+info.tel+"'>").show();
        }else{
          tel = dom.find(".idcard-tel").html("座机：<input type='text' style='cursor:pointer;' title='点击修改' readonly='readonly' value='点击添加'>").show();
        }
        dom.find(".idcard-email").text("邮箱："+(info.email ||""));
        dom.find(".idcard-pos").text("岗位："+(info.position ||""));
        dom.find(".idcard-orgname").text("部门："+(info.orgFullName ||""));
        if(info.avatar){
        ...
        $.facebox(dom.show());
        var h = dom.find(".pop-title").outerHeight()+dom.find(".pop-content").outerHeight()+dom.find(".green-btn").outerHeight() + 20;
        dom.parent().height(h);

        //输入事件绑定
        event(phone.children("input").bind("blur", function(){
          var p = $.trim($(this).val());
          if( p != info.phone){
            var valid = /^1\d{10}$/.test($(this).val());
            if(!valid){
              util.alert("提示","您输入的手机号有误", function(){
                phone.children("input").val(info.phone);
              });

            }else{
              updateInfo(uid, util.cookie("aid"), p , "", "");
            }

          }
          if($.trim($(this).val()) == ""){
            $(this).val(info.phone || "点击添加");
          }
          $(this).attr("readonly","readonly").css("border","none").css("cursor","pointer");
        }));
        event(tel.children("input").bind("blur", function(){
          var t = $.trim($(this).val());
          var valid = t && t != info.tel && t.length < 30;
          valid = valid && (/^(\+|\d)\d+(转|-|\s)?\d+$/.test(t) || /^\(\d{2,4}\)\d+(转|-|\s)?\d+$/.test(t));
          if(valid || t == ""){
            updateInfo(uid, util.cookie("aid"), "", t, "");
            $(this).val(t || info.tel ||"点击添加");
          }else{
            $(this).val(info.tel || "点击添加");
          }
          $(this).attr("readonly","readonly").css("border","none").css("cursor","pointer");
        }));
      }else{
    ...
    function event(jdom){
      jdom.bind("click", function(){
        if(!/^\d.*\d$/.test($(this).val())){
          ...
          }else if (this.select && typeof this.select == "function"){
            this.select();
          }
        }
        $(this).removeAttr("readonly").css("border","1px solid #aaa").css("cursor","auto");
      }).bind("keydown", function(evt){

        var allowedKey = [8, 37, 39, 46,107,109,189]; //delete  left  right  backspace + -
        if(allowedKey.indexOf(evt.keyCode) >= 0){
          return true;
        }
        //+
    ...
    function updateInfo(uid, aid, phone, tel, avatar){
      var url = "/api.action?ptype=iep_erp_set&uid="+uid+"&aid="+aid, update = false;
      if(phone){
        url = url + "&phone="+phone;
        update = true;
      }
	...
	require(["util"],function(util){
		if(text){
			...
			if(text.length > 200){
				...
				var url = "http://shop.chat.jd.com/client_advice/clientAdvice.action?token=" +
