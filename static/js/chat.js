		...
		send:function(msg){
            ...
            if(this.kind == "system"){
				...
				if(this.kind == "customer"){
					chat_single(this.msgId, this.conver, (util.filterMsg(inputText, false, false)),function(data){
//						chatAjax.showClientMsg(inputText,data);
					...
					chat_group(this.msgId, this.conver,(util.filterMsg(inputText, false, false)),function(data){
                        chatAjax.redirectLogin(data);
						chatAjax.afterSendMsg(inputText, data);
					}, function(){
						chatAjax.afterSendMsg(inputText);
					...
					chat_temp(this.msgId, this.conver, (util.filterMsg(inputText, false, false)),function(data){
                        chatAjax.redirectLogin(data);
						chatAjax.afterSendMsg(inputText, data);
					}, function(){
						chatAjax.afterSendMsg(inputText);
		...
		setRecentMsg:function(inputText,data){
			...
			if(recentmsg == null){
				DDstorage.set("chat_recentmsg_"+this.uid+"_"+this.conver,[clientmsg]);
			}else{
				recentmsg.unshift(clientmsg);
				DDstorage.set("chat_recentmsg_"+this.uid+"_"+this.conver,recentmsg);
			}
		},
...
//						console.log(data);
		...
		buildClientContent:function(){
			var cContent=['<div class="msg msg-self" time="" mid=""><div class="msg-avatar"> <img alt="" src="./img/img-avatar.png"><p></p></div><div class="msg-cont"></div> </div>'];
			return $(cContent.join(""));
		},
		buildLeftContent:function(){
			var lContent=['<li kind="" id="" conver="" class="rc-item"><div class="l"><img alt="" src="./img/img-avatar.png"><span class="i ui-hide"></span></div><div class="m">'+
		...
		afterSendMsg:function(inputText, data){
			if(data && data.body && data.body.mid){
				...
				//TODO 消息发送失败
				var ct = $(".msg-self[mid='ms']").not(".msg-failed").eq(0).addClass("msg-failed");
				ct.append("<div class='msg-clear'></div>");
				ct.append("<div class='msg-failed-tip'><span></span>消息发送失败，<a href='javascript:;'>点击重发</a></div>");
				//调整提示信息与内容的边距
				var h = ct.find(".msg-cont").outerHeight()- ct.find(".msg-avatar").outerHeight() + 10;
				if( h > 10){
					h = 10;
				}
		...
		resendMsg:function(text, target){
			...
			try{
				...
				jdom.find("img").each(function(){
					...
					$(this).replaceWith('<a rel="gallery" class="image-file" href="' + src + '">' + img + '</a>');
				});
				text = jdom.html();
			}catch(e){
			...
			if(this.kind == "customer"){
				chat_single(oldMsgId, this.conver, text ,function(data){
					if(data && data.body.mid){
						target.attr("mid",data.body.mid).removeClass("msg-failed").removeAttr("title");
