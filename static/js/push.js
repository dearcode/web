define(function(require, exports, module){
	...
	function poll(){
		...
		$.ajax({
			url:"/api.action?aid="+aid+"&from="+from+"&type=iq_offline_message_get",
			type:"GET",
			timeout: 60000,
			...
			error:function(){
				...
				if (errorTimes > 2 && $("#net-error").length == 0) {
					...
					if (state != "i i-offline") {
						$("#user_state").attr("last-status", state).attr("class", "i i-offline");
					}
                    stopPoll = true;
					util.alert("提示","<div id='net-error'>您的网络可能存在问题，请检查网络是否连接正常</div>", function(){
						errorTimes = 0;
    ...
    function errorReport(msg, ext){
        ...
        $.ajax({
            url:"/api.action?message="+data+"&uid="+uid+"&aid="+aid +"&type="+ext+"&cuid="+util.cookie("uid"),
            type:"GET"
        });
    }
    ...
    function popup4Relogin(msg){
        ...
        setTimeout(function(){
            ...
            util.alert("提示", msg || "登陆失效，请重新登陆", function(){
                util.cookie("uid", "", {expires:-1});
				util.cookie("aid", "", {expires:-1});
	...
	function parse(msgBean){
		...
		*/
		...
		if(msgBean.type == "message_notice"){
            msgBean.kind = "notice";
            var tim = util.formatDate(new Date(msgBean.body.datetime), "yyyy-MM-dd HH:mm:ss");
    ...
    function beforeParse(msgBean) {
          return (typeof msgBean.to != "undefined" && msgBean.to != uid && !$.isNumeric(msgBean.to)) && (typeof msgBean.from != "undefined" && msgBean.from != uid);
    }

	function updateUserInfo(msg){
		get_user_info(msg.from, function(json){
			var old = DDstorage.get(msg.from), avatar;
			if(old && old.uid){
				avatar = old.avatar;
			}
			if(json && json.code == 1){
				DDstorage.set(msg.from, json.body);
				if(json.body.avatar && json.body.avatar != avatar && json.body.avatar.indexOf("http://")>=0){
	...
	function parseChat(msgBean, offline){
		var conver = $(".panel-view").attr("conver"), userinfo, from , gid ,content, len;
        ...
        if(!$.isNumeric(t) && f != u && msgBean.type != 'message_notice'&& t.toLowerCase() != u){
		...
		//需要显示消息
		if((conver == from && !gid) || (conver == gid && gid)  ||(msgBean.type == "message_notice" && conver == msgBean.body.source) || (conver == t && msgBean.from == uid)){
			showMsg(msgBean, userinfo, true);
            if(conver == gid) {
                readMsg(conver, msgBean.kind, $(".panel-view").attr("kind"), msgBean.body.mid);
            }else {
	...
	function showGroupLeft(msg, info, userinfo){
		var gid = msg.body.gid, i, group, target;
		if(!info || !info.gid){
	...
	function buildRecentView(kind, conver, avatar,name, msg){
		...
		target.attr("kind", kind).attr("conver", conver).attr("id", "recent-contact-"+conver);
		...
		target.find(".r").html(util.formatDate(new Date(msg.body.datetime),"HH:mm:ss"));
	...
	function rebackMsgOffline(msgBean){
		...
		for(i=0; i<list.length; i++){
			...
			}else if(type == "message_group_invite" || type == "message_group_accept" || type =="message_group_decline"){
	...
	function showMsg(msg, userInfo, confirm){
		if(!userInfo){
			...
			if (msg.from != userInfo.uid) {
				console.log("--------------------------error\n"+msg+"\n"+userInfo+"\n-------------------------------------");
				var errorMsg = "get message="+JSON.stringify(msg)+", userInfo="+JSON.stringify(userInfo);
			...
			var jdom = buildContent(msg), lastTime, info;
			timeline(msg);
            if(msg.body.kind != 'voice'){
            ...
            if(msg.body.mode == 1001){
                ...
                if(msg.body.url && msg.type == "message_notice") {
                    content += "&nbsp;&nbsp;&gt;&gt;<a href='"+msg.body.url+"' target='_blank'>点击这里查看详情</a>"
                }
                if(msg.body.pic && msg.type == "message_notice") {
                    content += "<div><a rel='gallery' href='"+msg.body.pic+"'><img src='"+msg.body.pic+"' style='max-width:320px;' class='message-img'></a></div>"
                }
                jdom.find(".msg-cont").html(content);
            }
			if(userInfo.avatar){
  	...
  	function notify(msgBean, userinfo, getNotify){
	    ...
	    if(getNotify) {
	      ...
	      return {
	        ...
	        var notice = notification.noticeMsg(userinfo.avatar , key, userinfo.realname || msgBean.from, content);
	        var noticeCover;
	        if(msgBean.to == uid){
  ...
  function showMsgOutOfWrap(msg, userInfo) {
    ...
    var invisible = $(".msg-wrap").outerHeight() - $(".panel-msg .bd").scrollTop() > 2*$(".panel-msg .bd").height();
    if(invisible) {
      var $tipsWrap = $(".panel-msg .bar-wrap");
      var $tips = $('<div class="msg-invisible-wrap"><p><img class="msg-invisible-avatar" src="'+userInfo.avatar+'"/>'
        + notifyMsg.title + "：" + notifyMsg.content + '</p></div>')
      ...
      $(".panel-msg .bd").off("scroll").on("scroll", function() {
        if($(this).scrollTop() == $(".msg-wrap").outerHeight()-$(".panel-msg .bd").height()) {
          $tips.remove();
        }
      });
      //点击滚动到底部
	...
	function timeline(msgBean, history){
		var lastTime=$(".msg-wrap .msg:last").attr("time"), jdom, time, diff = 0;
        if(!msgBean) {
            return;
        }
        ...
        if(diff > 120000 && diff < 1000*60*60*24) {
            ...
            if($.isNumeric(msgBean.body.datetime)) {
                time = util.formatDate(new Date(msgBean.body.datetime), "yyyy-MM-dd HH:mm:ss");
            } else {
                time = util.formatDate(util.parseDate(msgBean.body.datetime), "yyyy-MM-dd HH:mm:ss");
            }
        }
        if(time) {
	...
	function getFileMsgHtml(msg){
		...
		if((msg.type && msg.type == "message_file") || (msg.body && msg.body.ptype == "message_file") || !msg.body.content){
			...
			var html = "<table><tr><td rowspan='2'><img src='./img/file2.png'></td>" +
					   ...
					   "<td rowspan='2'><a download='"+msg.body.name+"' style='margin:0 10px;' class='file_receive' target='_blank' href='"+
			           msg.body.url+"'> &nbsp;&nbsp;接收  </a></td>" +
			           "</tr><tr><td><span style='margin:0 10px;'>"+filesize+"</span></td></tr></table>";
			return html;
		}else{
			return "";
		}
	...
	function getUserInfo(key, erp){
		...
		if($.isNumeric(key) && erp){
			//在某些情况数据异常
			try{
				var list = DDstorage.get(key), i=0, user;
	...
	function buildContent(msg){
        var sContent='<div class="msg msg-other" time="" mid=""><div class="msg-avatar"><img src="./img/img-avatar.png"><p></p></div><div class="msg-cont"></div></div>';
        if(msg.from == uid) {
            sContent='<div class="msg msg-self" time="" mid=""><div class="msg-avatar"><img src="./img/img-avatar.png"><p></p></div><div class="msg-cont"></div></div>';
        }
		return $(sContent);
	}
    ...
    function bindMsgEvent(imgonload) {
		...
		if(imgonload) {
			$("#jd-recent-contacts .rc-item .rc-msg a").unbind("click").bind("click", function(event){
        ...
        $(".voice-msg").unbind("click").bind("click", function(){
            ...
            var _this = this;
            $(".voice-msg").removeClass("playing").removeClass("voice-playing").removeClass("voice-playing-self");
            if($(this).hasClass("voice-msg-self")) {
                $(this).addClass("voice-playing-self");
            }
            $(this).addClass("playing");
            if(player.length == 0){
                $("body").append("<div id='player_voice' style='width:0;height:0'></div>");
                player = $("#player_voice");
                player.jPlayer({
                    swfPath: "js",
                    supplied : "wav",
                    ...
                    error:function(){
                        $(".playing").removeClass("voice-playing").removeClass("voice-playing-self");
                    ...
                    ended: function(){
                        $(".playing").removeClass("voice-playing").removeClass("voice-playing-self");
                        $(".playing").parent().next().find(".voice-status .unread").addClass("read");
                    }
                });
            } else {
