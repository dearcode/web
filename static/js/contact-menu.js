	...
	if (name == "oper-sendmsg") {
		...
		for (var i = 0; i < mods.length; i++) {
			str += '<li rel="g" class="exist-item ' + (i == 0 ? "selected" : "") +
				'" labelId="' + mods.eq(i).attr("labelId") + '"><span class="i"></span>' +
				mods.eq(i).attr("name") + '</li>';
		}
		...
		addContact.find("li,a").on("click", function() {
			...
			if (rel == "ok") {
				move_friend(labelId, data, function(data) {
					...
					allcon.find(".allcontacts").text(Number(allcon.find(".allcontacts").text()) +
						1);

					var mod1 = $("#contactmod-" + slabelId);
					var allcon1 = mod1.parent().parent().find(".hd");
					allcon1.find(".allcontacts").text(Number(allcon1.find(".allcontacts")
						.text()) - 1);

					if (contact.find(".l img").attr("status") != "off") { // 更正在线数
						allcon.find(".online").text(Number(allcon.find(".online").text()) +
							1);

						allcon1.find(".online").text(Number(allcon1.find(".online").text()) -
							1);
						mod.prepend(contact);
					} else {
						mod.append(contact);
...
function _show_card_evet(idcard, userinfo) {
	...
	var h = idcard.find(".pop-title").outerHeight() + idcard.find(".pop-content").outerHeight() +
		idcard.find(".blue-btn").outerHeight() + 20;
			...
			if (mods.length == 0) {
				get_contact_list(function(data) {
					...
					for (var i in groups) {
						var group = groups[i];
						str += '<li rel="g" class="exist-item ' + (i == 0 ? "selected" : "") +
							'" labelId="' + group.id + '"><span class="i"></span>' + util.filterMsg(
								group.name) + '</li>';
					}
					addContact.find(".exist-wrap").html(str);
					jQuery.facebox(addContact.show());
			...
			if (data.type == "message_ack") {
				get_user_info(pin, function(result) {
					...
					get_contact_status(pin, function(r) {
						...
						var str = ' <li class="item" conver="' + pin + '" id="contact-' +
							pin + '" kind="customer">' + '<div class="l">' +
							'<img src="./img/img-avatar.png" status="' + offline + '"/>' +
							'</div>' + '<div class="m">' +
							'<div class="nickname"><span title="' + userinfo.realname + '">' +
							userinfo.realname + '</span><span class="' + get_status_class(
								offline) + '"></span></div>' + '<div class="rc-msg wto"></div>' +
							'</div><div class="r">' + '<span class="i i-ctt" data="' + pin +
							'" labelId="' + labelId + '"></span>' + '</div></li>';
						...
						if (offline == "off") {
							...
							dom.find(".wto").attr("title", result.body.signature).text(result
								.body.signature);
