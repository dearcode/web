define(function(require, exports, module) {
    ...
    function buildView() {
        ...
        $(document).bind("afterReveal.facebox", function() {
            ...
            img.onload = function() {
                ...
                var h = $(".upload-avatar:visible").height() *
                    imgWidth / imgHeight;
            ...
            function updateView(coords) {
                ...
                if (imgWidth == 0 || imgHeight == 0) {
                    imgWidth = $(".view-avatar:visible img")[0]
                        .naturalWidth || $(
                            ".view-avatar:visible img").width();
                    imgHeight = $(".view-avatar:visible img")[0]
                        .naturalHeight || $(
                            ".view-avatar:visible img").height();
                }
                //				console.log(rx);
                //				console.log(ry);
    ...
    function events() {
                ...
                reader.onload = function() {
                    ...
                    img.onload = function() {
                        var h = this.height / this.width *
                            $(".view-avatar:visible").width(),
                            h2 = this.height / this.width *
                            ($(".upload-avatar:visible")
                                .width() * 1.2),
                            w2 = this.width / this.height *
                            h2;
                        imgHeight = this.height;
                        imgWidth = this.width;
                        jcrop_api.ui.holder.height(h);
                        $(".view-avatar img").attr(
                            "src", img.src).height(
                            h);
                        $(".upload-avatar img").attr(
                            "src", img.src).height(
                            h2).width(w2);
                        //						if(context){
                        //							context.clearRect(0, 0, canvas.width, canvas.height);
        ...
        $(".avatar-save").click(function() {
            ...
            req.x = parseInt(x, 10);
            req.y = parseInt(y, 10);
            req.height = height > 0 ? parseInt(height, 10) :
                parseInt(imgHeight, 10);
            req.width = width > 0 ? parseInt(width, 10) :
                ...
                success: function(json) {
                    if (json && json.url) {
                        ...
                        $("#user-avatar img").css(
                            ...
                            "5px").css("height",
                            "64px");
                        var self = DDstorage.get(
                            util.cookie("uid"));
                        if (self) {
                            self.avatar = json.url;
                            DDstorage.set(util.cookie(
                                    "uid"),
                                self);
                        }
    ...
    function init() {
        ...
        if (obj && obj.avatar) {
            $("#user-avatar img").attr("src", obj.avatar).css(
                "borderRadius", "5px").css("height", "64px");
