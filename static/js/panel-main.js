...
//											console.log(data)
								...
								get_user_info(pin, function(data) {
											...
											dom.find(".wto").attr("title", data.body.signature)
													.text(data.body.signature);
										});
							}
							$("#j-tabPanelMainHd").find("li.selected").click();
						} else {
				...
				for (var i = 0; i < inputs.length; i++) {
					...
					if (name && labelId && name != inputs.eq(i).attr("rel")) {
						addorModify_friend_label(labelId, name, 0, function(data) {
									$("#jd-contact").find("div[labelid=" + data.body.id + "]").attr("name",
											data.body.name).find(".l span:eq(1)").text(data.body.name);
								}, function(data) {
//									console.log(data);
								});
					}
