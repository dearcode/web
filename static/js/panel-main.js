...
(function() {
	...
	$("#j-tabPanelMainBd").on("click", ".item,.rc-item,.g-item", function(e) {
		...
		if (!is_selected) {
			$("#jd-recent-contacts,#jd-contact,#panel-search-re").find("li.selected").removeClass("selected");
	...
	function doSearchResult(data) {
		...
		for (var i in arr) {
			...
			str += ' <ul class="rc-wrap"><li class="rc-item" id="search-re-' + obj.uid + '" conver="'
			+ obj.uid + '" kind="customer"><div class="l">'
			...
			+ (obj.orgFullName ||"") + '</span></div>' + '</div><div class="r" pos="' + (obj.position ||"")
			+ '" email="' + (obj.email||"") + '" phone="' +(obj.phone ||"") + '" uid="' + obj.uid
			+ '" realname="' + obj.realname
			+ '" tel="'+ (obj.tel ||"")+'" orgFullName="'+(obj.orgFullName ||"")+'"><div><span class="i opr" rel="view" title="名片"></span></div>'
		...
		for(var i=0; i<groups.length; i++) {
			...
			+'      </div>' ;
			var ingroup = $("#jd-group-items").find(".g-item[conver='"+groups[i].gid+"']").length > 0;
			if(ingroup) {
				str += '<div class="r" style="width:0px;" gid="'+groups[i].gid+'" code="'+(groups[i].sCode ||"")+'" name="'+gname+'" style="display:none;"></div>'
			} else {
				str += '<div class="r" style="width:42px;" gid="'+groups[i].gid+'" code="'+(groups[i].sCode ||"")+'" name="'+gname+'" ><div style="display:none;"><span class="i opr" rel="view" title="名片"></span></div><div><span class="i opr" rel="add"></span></div>'
			}
			str +=  '  </li></ul>'
		}
		searchele.html(str);
	...
	$("#panel-search-re").on("click", ".opr", function() {
		...
		if ($(this).attr("rel") == "add") {
            if(kind == "discussion_group") {
                addGroup(datadom.attr("gid"), kind, datadom.attr("code"), datadom.attr("name"));
                return;
            }
				...
				get_contact_list(function(data) {
					...
					for (var i in groups) {
						...
						str += '<li rel="g" class="exist-item ' + (i == 0 ? "selected" : "") + '" labelId="'
							+ group.id + '"><span class="i"></span>' + gname + '</li>';
					}
    ...
    function addGroup(gid, kind, code, name) {
        ...
        function joinGroup(gid, code, kind, name){
           if(code) {
               var util = require("util");
               var html = '<div><div>请输入加群验证码</div><div><input style="border: 1px solid #A3A1A1;width:240px;height:24px;" type="text" name="scode"></div></div>';
	...
	var search_addcontact = function(that) {
		...
		var labelId = pa.find(".exist-wrap li.selected").attr("labelId"), rel = $(that).attr("rel"), pin = $(that).parent().attr("data"), slabelId = $(that).parent().attr("labelId");
		if (labelId == slabelId) {
			...
			add_friend(pin, labelId, function(data) {
				if (data.type == "message_ack") {
					...
					var str = ' <li class="item" conver="' + pin + '" id="contact-' + pin
						+ '" kind="customer">' + '<div class="l">' + '<img src="'
						+ userdom.find(".l img").attr("src") + '" status="' + offline + '"/>' + '</div>'
						+ '<div class="m">' + '<div class="nickname"><span title="' + userinfo.realname
						+ '">' + userinfo.realname + '</span><span class="'
						+ userdom.find(".nickname span:eq(1)").attr("class") + '"></span></div>'
					...
					if (offline == "off") {
						...
						get_user_info(pin, function(data) {
							...
							dom.find(".wto").attr("title", data.body.signature).text(data.body.signature);
						});
					}
					$("#j-tabPanelMainHd").find("li.selected").click();
				} else {
	...
	$("#jd-add-group").on("click", function() {
		...
		addNewContactGroup.find(".btn-wrap").attr("seq", $(this).attr("seq")).attr("ver", $(this).attr("ver"));
		...
		addNewContactGroup.find("a").on("click", function() {
			if ($(this).attr("rel") == "save" && !$(this).attr("disabled")) {
				...
				if(!name){
					var tip =  $(this).parents("div[id='addNewContactGroup']").find(".acg-tip");
					var text = $("#addNewContactGroup").find(".acg-tip").text();
					var html = "<span style='color:red;'>分组名不能为空</span>";
					tip.html(html);
					$(this).parents(".add-contact-group").find(".group-name").focus(function(){
						tip.html(text);
					});
					return;
				}
	...
	$("#jd-modify-group").on("click", function() {
		...
		for (var i = 0; i < mods.length; i++) {
			...
			if (mods.eq(i).attr("type") == "SYSTEM") {
				str += '<label class="gm-item"><span>分组名：</span>' + '<input type="text" disabled="disabled" value="'
						+ name + '"></label>';
			} else {
				str += '<label class="gm-item"><span>分组名：</span>' + '<input data="' + mods.eq(i).attr("labelId")
						+ '" type="text" rel="' + name + '" value="' + name + '"><span class="i i-del-group" labelId="'
						+ mods.eq(i).attr("labelId") + '"></span></label>';
			}
		}
		...
		groupMgr.find(".i-del-group").on("click", function() {
			var util = require("util"), labelId = $(this).attr("labelId"), that = this;
			if (!labelId) {
				return;
			}
			util.confirm("提示", '<div class="pop-content">' + '<div class="i i-del-gpcm"></div>'
							+ '<div class="del-gpcm-tip">选定分组将被删除，组内联系人将被移至默认分组' + '“常用联系人”里，您确定要删除该分组吗？</div></div>',
							function() {
		...
		groupMgr.find("a").on("click", function() {
			if ($(this).attr("rel") == "save") {
				...
				for (var i = 0; i < inputs.length; i++) {
					var name = inputs.eq(i).val(), labelId = inputs.eq(i).attr("data");
					if (name && labelId && name != inputs.eq(i).attr("rel")) {
						addorModify_friend_label(labelId, name, 0, function(data) {
							$("#jd-contact").find("div[labelid=" + data.body.id + "]").attr("name", data.body.name).find(".l span:eq(1)").text(data.body.name);
						}, function(data) {
							//console.log(data);
						});
					}
