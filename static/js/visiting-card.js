...
define(function(require, exports, modules) {
    ...
    function _show(userinfo) {
        ...
        if (userinfo.signature) {
            var signatureDomHtml =
                "<li class='idcard-signature wto' title='" + userinfo.signature +
                "'>签名：" + userinfo.signature + "</li>";
            $(signatureDomHtml).insertAfter(info.find(".idcard-pos"));
        }
        info.find(".idcard-orgname").html("部门：<span>" + (userinfo.orgFullName ||
            "") + "</span>");
        ...
        info.find(".idcard-orgname").append(
            "<img class='search-dept' style='float:none;width:44px;height:22px;vertical-align: middle;' title='搜索同部门联系人' src='/static/img/search.png'>"
        );
        ...
        inContact(userinfo.uid, function(id) {
            ...
            dom.click(function() {
                add2Contact(userinfo.uid, list, info,
                    userinfo);
            });
        });
    ...
    function add2Contact(id, list, dom, userinfo) {
        ...
        for (i = 0; i < list.length; i++) {
            group = list[i];
            str += '<li rel="g" class="exist-item ' + (i == 0 ?
                    "selected" : "") + '" labelId="' + group.id +
                '"><span class="i"></span>' + util.filterMsg(group.name) +
                '</li>';
        }
        ...
        addContact.find("a[rel='ok']").click(function() {
            var req = {
                ...
                version: "1.0",
                body: {
                    labelId: addContact.find(
                        ".exist-wrap .selected").attr(
                        "labelid")
                }
            };
            $.ajax({
                    ...
                    if (json && json.body) {
                        ...
                        if (target.length > 0) {
                            getStatus(id, function(
                                ...
                                if (!online) {
                                    html +=
                                        '<span class="gs-mask" title="离线" style="width: 40px; height: 40px; top: 2px; left: 2px; "></span>';
                                }
                                html +=
                                    '</div><div class="m"><div class="nickname">';
                                html +=
                                    '<span title="' +
                                    name +
                                    '">' +
                                    ...
                                    '</span>';
                                html +=
                                    '<i class="offline-text"></i><span class=""></span></div><div class="rc-msg wto"></div></div>';
                                html +=
                                    '<div class="r"><span class="i i-ctt" data="' +
                                    id +
                                    '" labelid="' +
                                    req.body
                                    .labelId +
        ...
        util.confirm("提示", "确实要删除该联系人？", function() {
            ...
            $.ajax({
                ...
                success: function(resp) {
                    if (resp) {
                        var target = util.contactDom(
                            id).parent();
                        var tnum = target.parent().prev();
                        util.contactDom(id).remove();
                        tnum.find(".allcontacts").text(
                            target.children().length
                        );
                        tnum.find(".online").text(
                            target.find(
                                "img[status='chat']"
                            ).length);
                    }
